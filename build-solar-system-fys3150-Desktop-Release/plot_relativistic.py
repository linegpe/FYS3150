...
# f = open("perihelion.dat","r")
# lines = f.readlines()

# for line in lines:
# 	relativistic.append(line.split('   ')[0])
# f.close()

data = np.loadtxt("perihelion.dat")

relativistic = data[0::1]

data2 = np.loadtxt("perihelion_nonrel.dat")
...
for i in range(1,N-1):
	relativistic[i] = relativistic[i]*3600*180/np.pi
	nonrel[i] = nonrel[i]*3600*180/np.pi

plt.plot(x,relativistic,label="Relativistic")
plt.plot(x,nonrel,label="Non-relativistic")
plt.legend()
plt.xlabel("Time [years]",fontsize=15)
plt.ylabel("Angle [sec. of arc]",fontsize=15)
plt.show()
